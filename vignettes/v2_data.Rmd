...
**Working with own files**


If you have your own files, you should just specify a path to your file or to a folder with files. Then load data using `repLoad`:

```{r, eval=F}
#file_path is a path to own folder or file
...
You could find the path to a folder with example files [here](https://github.com/immunomind/immunarch/tree/master/inst/extdata/io). 

Let's check your working folder
```{r, eval=F}
getwd()
```

Let's download file `'Sample1.tsv.gz'` in the working folder from [here](https://github.com/immunomind/immunarch/tree/master/inst/extdata/io) and load this file:

```{r, eval=F}
immdata <- repLoad('Sample1.tsv.gz')
```

You could also download all files in the `'example'` folder and load all of them:

```{r, eval=F}
immdata <- repLoad('example')
```

Also, example data is downloaded with `immunarch` packages. You could load whole example data using the following command:

```{r, eval=F}
#path to folder with example data
