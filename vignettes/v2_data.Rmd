...
Let's download file `'Sample1.tsv.gz'` in the working folder from [here](https://github.com/immunomind/immunarch/tree/master/inst/extdata/io) and load the file:

```{r, eval=F}
immdata <- repLoad('Sample1.tsv.gz')
```

You could also download all files to the `'example'` folder and load all of them:
