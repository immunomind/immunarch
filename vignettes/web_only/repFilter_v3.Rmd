...
```{r example-2.5}
#.method = "by.meta", .query = list(Age = interval(15, 23), Lane = include('AB')), .match = "substring"
immdata$meta %>% filter(Age > 15 & Age < 23 & Lane == 'AB')
names(immdata$data)
```
...
Note, filtering `by.clonotype` or `by.cl` works within repertoire(sample). Repertoire(sample) could be removed from your data only in case if all clonotypes in sample do not satisfy the condition: 
...
Filter out clonotypes with 1 clones from `immdata`:
...
In method `by.clonotype` or `by.cl`, there are an extra argument `.match`. The `.match` argument can has the following values: 
 - `exact` - looks for exact match in gene names
 - `substring`- looks for substring in gene names
 - `startswith` - looks for gene names starting with the some pattern
