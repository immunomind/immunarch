...
T cells that are able to recognize identical  epitopes were shown to have high similar TCR (Glanville et al. 2017; Dash et al. 2017). Clustering allows finding high similar TCR in repertoires that are likely to recognize the same antigens. 
...
CDR3 region is the most variable region in BCR and TCR. This region is commonly used for calculating the distance between sequences. However, other regions could also be used for calculating this distance — like full nucleotide reads. You can use any user-defined column of sequences to calculate the distance, since column selection depends solely on your research purpose. For example, the use of ‘nt’ sequences results in more conservative clustering. 
...
The default approach is to use a Hamming distance. When sequences have different lengths the distance between them equals infinity. Use this approach if you want to take into consideration only the point mutations excluding the indels. 
...
When you want to take into consideration not only the single mutations but also indels, Levenshtein distance is a more appropriate method to use.
...
You define in advance the minimum percentage of similarity for the sequences in your cluster. For example, you want to find cluster of BCRs that not only have the same [V.name](http://V.name), [J.name](http://J.name), and CDR3 length — but also have more than 90 percent match in their CDR3  
...
You directly determine the number of mismatched nucleotides. In case of TCR-recognising the same epitope, you use 1 amino acid mismatch in CDR3 sequences.

```{r example 9}
#clustering TCR 
clustTCR <- seqCluster(TCRdata, distTCR, .fixed_threshold = 1) 
...
```{r example 10}
#clustering TCR 
clustTCR <- seqCluster(TCRdata, distTCR, .nt_similarity = 15) 
...
If you have made it to this part, then you've successfully clustered your sequences. Congratulations! Now you have a new column `“cluster”` in your data sheet. You can now generalize the concept of clonotype based on sequence similarity and consider clusters as clonotypes.
